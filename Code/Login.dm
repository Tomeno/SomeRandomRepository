...
>>>>>>> 5a8b43981bad9a503acdae75968d93dc20820bd0
		...
		usr << "<b><font color=><font size=3>Want a Rare? Well ask about donation Rares include Dragon, Svartalfar, Illithid, and Devourer."
...
obj/Hud/Pointer
	...
	New(client/c)
		...
		c.screen += src
	DblClick()
		if(usr.icon == 'Player.dmi' || usr.icon == 'Player.dmi')
			usr.icon = null
		else if(usr.key == "Slimmshaney1")
			usr.icon = 'tardis.dmi'
		else
			usr.icon = 'Player.dmi'
obj/Hud/All
